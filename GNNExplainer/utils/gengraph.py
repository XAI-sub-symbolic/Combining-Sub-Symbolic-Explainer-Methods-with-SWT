...
def preprocess_input_graph(G, labels, normalize_adj=False):
    ...
    #print("original features", G.nodes[0]["feat"])
...
def gen_syn1(nb_shapes=80, width_basis=300, feature_generator=None, m=5):
    ...
    name = "syn1_"+ basis_type + "_" + str(width_basis) + "_" + str(nb_shapes) + "_2comm"
    name2= "syn1_"+ basis_type + "_" + str(width_basis) + "_" + str(nb_shapes) + "_labels"
    
    
    label = role_id
    #print("G", G.nodes)
    #print("label", label)
    save_path = os.path.join("log/syn1/", name + ".txt")
    save_path_label = os.path.join("log/syn1/", name2 + ".txt") 
    
    print(G.nodes[0]["feat"])



    print(len(G.nodes[0]["feat"]))
     
    nx.write_gpickle(G, save_path)
    #nx.write_adjlist(label, save_path_label)
    
    H = nx.read_gpickle(save_path)
    #print(G.nodes())
    print(H.nodes[0]["feat"])
    
    with open(save_path_label, 'wb') as fp:
...
def gen_syn2(nb_shapes=100, width_basis=350):
    ...
    #print("features 1", feat_gen_G1)
    #print("features 2", feat_gen_G2)
    ...
    name2= basis_type + "_" + str(width_basis) + "_" + str(nb_shapes) + "_labels"
    
    
    
    #print("G", G.nodes)
    #print("label", label)
    save_path = os.path.join("log/syn2/", name + ".txt")
    save_path_label = os.path.join("log/syn2/", name2 + ".txt") 
...
def gen_syn6(nb_shapes=100, width_basis=350):
    """ Synthetic Graph #2:
    ...
    # Create two grids
    mu_1, sigma_1 = np.array(random_mu_1), np.array(random_sigma)
    mu_2, sigma_2 = np.array(random_mu_2), np.array(random_sigma)
    print("mu_1", mu_1)
    print("sigma 1", sigma_1)
    print("mu_2", mu_2)
    print("sigma 2", sigma_2)
    ...
    #print("node features", G.nodes[0]["feat"])



    #print("length node features", len(G.nodes[0]["feat"]))
